...
CREATE VIEW [dbo].[V__EVault_MSP_Partner_Bookings] AS
...
SELECT
	...
	,		CASE
				WHEN [na_user].[NAME] IN ('paul binder','kelly o''connell') THEN [evault_booking].[customer]
				ELSE COALESCE([evault_booking].[end_user], [evault_booking].[customer])
			END [MSP_Partner]
	,		CASE
				WHEN [na_user].[NAME] IN ('paul binder','kelly o''connell') THEN [na_account].[GEO__C]
				ELSE [emea_account].[GEO__C]
			END [MSP_Partner_GEO]
	,		CASE
				WHEN [na_user].[NAME] IN ('paul binder','kelly o''connell') THEN [na_account].[BILLINGCOUNTRY]
				ELSE [emea_account].[BILLINGCOUNTRY]
			END [MSP_Partner_Country]
	,		CASE
				WHEN [na_user].[NAME] IN ('paul binder','kelly o''connell') THEN [na_account].[BILLINGSTATE]
				ELSE [emea_account].[BILLINGSTATE]
			END [MSP_Partner_State]
	,		CASE
				WHEN [na_user].[NAME] IN ('paul binder','kelly o''connell') THEN [evault_booking].[Customer_SFDC_ID]
				ELSE COALESCE([evault_booking].[End_User_SFDC_ID], [evault_booking].[Customer_SFDC_ID])
			END [MSP_Partner_SFDC_D]
	,		CASE
				WHEN [na_user].[NAME] IN ('paul binder','kelly o''connell') THEN [na_user].[NAME]
				ELSE [emea_user].[NAME]
...
('CUST9992417 BCS Technologies S.A.') -- added on 10/14/2016 requested by Katie Moore
) [emea_msp_partner_list] ([name]) ON LEFT([emea_msp_partner_list].[name],11) = [emea_account].[NETSUITE_CUSTOMER_NUMBER__C]



LEFT OUTER JOIN [dbo].[V__EVault_MSP_Partner_Bookings__MSP_Model_Details] [MSP_type] WITH (NOLOCK) ON [MSP_type].[Account_Name] = CASE
				WHEN [na_user].[NAME] IN ('paul binder','kelly o''connell') THEN [evault_booking].[customer]
				ELSE COALESCE([evault_booking].[end_user], [evault_booking].[customer])
			END

WHERE 1=1
AND (([na_user].[NAME] IN ('paul binder','kelly o''connell') OR [emea_msp_partner_list].[name] IS NOT NULL) OR [evault_booking].[Finance_Product_grouping]='royalty')
