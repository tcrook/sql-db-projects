...
CREATE VIEW [dbo].[V__NS_Sales_Order_Line_Items] AS
SELECT
...
,	[t].ORIGINALDEBOOKING_NS_ORDER_ID
,	[org_ot].[LIST_ITEM_NAME] Debooked_order_type
...
,	CAST([opp].[CLOSEDATE] AS DATE) [Opp_Close_Date]
...
LEFT OUTER JOIN dbo.NSCARB_TRANSACTIONS org_so ON org_so.TRANSACTION_ID = [t].ORIGINALDEBOOKING_NS_ORDER_ID
LEFT OUTER JOIN [NSCARB_order_type] [org_ot] WITH (NOLOCK) ON [org_ot].[LIST_ID] = [org_so].[ORDER_TYPE_ID]
...
--AND [t].[TRANID]>=110010232 -- SOs in carbonite production instance start with this tranid.
